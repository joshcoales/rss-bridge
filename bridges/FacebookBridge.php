...
TAG;
	 ...
	 * @param $user string User input for username.
	 ...
	 * @param $content string The page content string
	 * @return string The page content with facebook redirect links stripped out
	 */
		...
		return preg_replace_callback('/ href=\"([^"]+)\"/i', function ($matches) {
			if (is_array($matches) && count($matches) > 1) {
				...
				if (strpos($link, 'facebook.com/l.php?u=') !== false)
					$link = urldecode(
						extractFromDelimiters($link, 'facebook.com/l.php?u=', '&')
					);
	 ...
	 * @param $content string The page content string
	 * @return string Page content with facebook tracking codes stripped out
	 */
...
<p><img src="data:image/png;base64,{$img}" alt="Captcha"/></p>
<p><b>Response:</b> 
<input name="captcha_response" placeholder="please fill in" />
	...
	private function handleCaptchaResponse()
	{
		if (isset($_POST['captcha_response'])) {
			...
			if (isset($_SESSION['captcha_fields'], $_SESSION['captcha_action'])) {
				...
				$captcha_fields['captcha_response'] = preg_replace(
					'/[^a-zA-Z0-9]+/', '', $_POST['captcha_response']
				);
	...
	private function collectUserData()
	{
		...
		if ($loginForm != null) {
			returnServerError(
				...
				This is not supported by RSS-Bridge.'
			);
		}
		...
		if (isset($element)) {
			...
			$author = str_replace(
				' - Posts | Facebook', '', $html->find('title#pageTitle', 0)->innertext
			);
